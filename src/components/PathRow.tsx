...
export function PathRow({ funnel, defaultExpanded = false, onToggleActive }: PathRowProps) {
  ...
  return (
    <div
      className={`card-elevated mb-3 overflow-hidden transition-all duration-300 ${
        inactive ? "opacity-50 grayscale" : ""
      }`}
    >
      {/* Main row */}
      <div
        className="flex items-center gap-3 px-4 py-3.5 md:px-5 cursor-pointer group"
      ...
      >
        ...
        {/* Keyword badge */}
        <span
          className={`shrink-0 inline-flex items-center px-3 py-1.5 rounded-xl text-[11px] font-bold uppercase tracking-wider ${
            inactive
        ...
        <div className="hidden md:flex items-center gap-4 shrink-0">
          <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground" title="Контент">
            ...
            <span className="font-medium">{funnel.contentCount}</span>
          </div>
